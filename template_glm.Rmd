...
We note that in the case of MLR, the maximum likelihood estimator is the same as the least squares estimator. 
...
Which we test using the test statistics
...
The residual sum of squares `SSE` is the sum of squares  $\sum^n_{i=1} (y_i - \hat{y}_i)^2$. The degrees of freedom for this model is the number of observations minus the number of fitted parameters, $n - p$. The total sum of squares `SST` is similarly defined as $\sum^n_{i=1} (y_i - \bar{y})^2$. The regression sum of squares is the difference $SST - SSE$. Under $H_0$ we have that
